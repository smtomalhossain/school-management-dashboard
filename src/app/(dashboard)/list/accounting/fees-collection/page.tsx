...
type Fees = {
  ...
  paidAmount: number;  // Keep number here
  photo: string;
  class: string;
  status: string;
  date: string;
...
const FeesPage = () => {
      ...
      if (res.status === 200) {
        ...
        const schoolList: Fees[] = data.map((item: any) => {
          return {
            ...
            paidAmount: Number(item.paidAmount) || 0, // CONVERT to number here!
    ...
    >
      <td className="flex items-center gap-4 p-4">
        <Image
          ...
          // Next.js Image doesn't support onError, so for fallback image you might consider using next/image loader or other technique.
          // Here, we assume photo URL is always valid or fallback is set in mapping.
        />
        <Link href={`/list/list/accounting/fees-collection/${item.id}`}>
          <h3 className="font-semibold cursor-pointer hover:underline">{item.name}</h3>
        </Link>
      </td>
  ...
  for (let key in monthly) {
    if (monthly.hasOwnProperty(key)) {
      monthlyData.push(months[Number(key)] + " = " + monthly[key]);
    }
  }
